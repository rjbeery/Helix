 ...
 */
export class BedrockEngine implements Engine {
  ...
  constructor(model: string, config: EngineConfig & { region?: string } = {}) {
...
export const createBedrockEngine = {
  claudeV2: (config?: EngineConfig) => new BedrockEngine('anthropic.claude-v2', config),
  claude3Sonnet: (config?: EngineConfig) => new BedrockEngine('anthropic.claude-3-sonnet-20240229-v1:0', config),
  titan: (config?: EngineConfig) => new BedrockEngine('amazon.titan-text-express-v1', config),
};
