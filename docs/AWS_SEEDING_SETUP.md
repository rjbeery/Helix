...
aws secretsmanager create-secret `
  ...
  --secret-string '{\"email\":\"admin@example.com\",\"password\":\"REPLACE_WITH_STRONG_PASSWORD\"}'
```
...
aws lambda update-function-configuration `
  ...
  --region $region `
  --function-name helixai-api `
  --environment "Variables={ADMIN_EMAIL=admin@example.com,ADMIN_PASSWORD=REPLACE_WITH_STRONG_PASSWORD,NODE_ENV=production}"
```
...
aws ssm put-parameter --name "/helix/prod/ADMIN_EMAIL" --value "admin@example.com" --type "SecureString" --profile $profile --region $region
aws ssm put-parameter --name "/helix/prod/ADMIN_PASSWORD" --value "REPLACE_WITH_STRONG_PASSWORD" --type "SecureString" --profile $profile --region $region
